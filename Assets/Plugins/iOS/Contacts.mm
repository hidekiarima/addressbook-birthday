...
extern "C" {
    ...
    void contact_loadName( ContactItem* c )
    {
        ...
        NSString* birthDay = (__bridge NSString *)(ABRecordCopyValue(c->person, kABPersonBirthdayProperty));
        
        NSString* f = ( firstName == NULL)?[[NSString alloc]init]:firstName ;
        NSString* s = ( lastName == NULL)?[[NSString alloc]init]:lastName ;
        NSString* b = ( birthDay == NULL)?[[NSString alloc]init]:birthDay ;
        
        c->name = [NSString stringWithFormat:@"%@ %@ BD:%@",s,f,b];
